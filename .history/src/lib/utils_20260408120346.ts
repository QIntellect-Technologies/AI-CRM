...
export function getAvatarUrl(name: string = '', id: string | number = 1): string {
  ...
  const femaleNames = [
    ...
    'maria', 'emily', 'jessica', 'huda', 'maha', 'mona', 'nada', 'rasha',
    'samira', 'tariq' // Wait tariq is male. Remove tariq.
  ];
  
  // Real check
  const femaleList = ['sara', 'sarah', 'reem', 'fatima', 'aisha', 'layla', 'maryam', 'zainab', 'khadija', 'amna', 'shamsa', 'maha', 'hind', 'nouf', 'salma', 'jasmine', 'maria', 'emily', 'jessica', 'huda', 'mona', 'nada', 'rasha', 'samira'];
