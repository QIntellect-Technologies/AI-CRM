  ...
  return (
    <div className="flex h-screen bg-navy-900 font-sans text-gray-200 overflow-hidden selection:bg-cyan-glow/30 selection:text-cyan-glow relative">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden z-10 relative">
        {/* Top Header Bar */}
        <header className="h-20 flex items-center justify-between px-8 shrink-0 border-b border-white/5 bg-navy-800/40 backdrop-blur-md">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              ...
              {branchOpen && (
                <div className="absolute top-full mt-2 w-48 glass-panel rounded-xl py-2 z-50">
          ...
          <div className="flex items-center gap-4">
            <div className="relative group cursor-pointer p-2">
              <Bell className="text-gray-400 hover:text-cyan-glow transition-colors" size={20} />
              <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-red-500 rounded-full border border-navy-900 animate-pulse"></span>
              <div className="absolute top-full right-0 mt-2 w-80 glass-panel rounded-xl p-4 shadow-xl opacity-0 group-hover:opacity-100 group-hover:pointer-events-auto pointer-events-none transition-all z-50">
                ...
                <div className="space-y-3">
                  {[
                    { title: 'Follow-up Due', text: 'High priority lead waiting for response', time: '10 min ago', color: 'red' },
                    { title: 'Patient Inactive', text: 'Sarah Ahmed inactive for 45 days', time: '1 hour ago', color: 'yellow' },
                    { title: 'Campaign Success', text: 'TikTok ad generated 15 new leads', time: '2 hours ago', color: 'emerald' }
                  ].map((notif, i) => (
                    <div key={i} className="flex items-start gap-3 p-2 hover:bg-white/5 rounded-lg transition-colors">
                      <div className={cn("w-2 h-2 rounded-full mt-1.5 shrink-0 shadow-[0_0_8px]", `bg-${notif.color}-500 shadow-${notif.color}-500/50`)}></div>
                      <div>
                        <p className={cn("text-sm font-medium", `text-${notif.color}-400`)}>{notif.title}</p>
            ...
            <button className="relative w-10 h-10 flex items-center justify-center rounded-full glass-input hover:bg-white/5 transition-all">
              ...
              <span className="absolute top-2 right-2 w-2 h-2 bg-[#e6396b] rounded-full shadow-[0_0_8px_rgba(230,57,107,0.8)]"></span>
            </button>
