  ...
  return (
    <div className="flex h-screen bg-navy-900 font-sans text-gray-200 overflow-hidden selection:bg-cyan-glow/30 selection:text-cyan-glow relative">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden z-10 relative">
        {/* Top Header Bar */}
        <header className="h-24 flex items-center justify-between px-8 shrink-0 relative z-50 bg-gradient-to-b from-navy-900/90 to-navy-800/40 backdrop-blur-2xl border-b border-light-glow/5 shadow-[0_4px_30px_rgba(0,0,0,0.2)]">
          
          {/* Breadcrumb & Title */}
          <div className="flex items-center gap-5">
            <div className="p-3 bg-gradient-to-br from-cyan-glow/20 to-blue-500/20 rounded-2xl border border-cyan-glow/30 shadow-[0_0_20px_rgba(0,212,255,0.2)]">
              <LayoutDashboard size={24} className="text-cyan-glow drop-shadow-[0_0_8px_rgba(0,212,255,0.8)]" />
            </div>
            <div>
              <div className="flex items-center gap-2 text-[10px] font-mono uppercase tracking-widest text-gray-500 mb-1">
                <span className="hover:text-gray-300 cursor-pointer transition-colors">Workspace</span>
                <ChevronDown size={10} className="text-gray-600 -rotate-90" />
                <span className="text-cyan-glow bg-cyan-glow/10 px-2 py-0.5 rounded-full border border-cyan-glow/20 shadow-[0_0_8px_rgba(0,212,255,0.2)]">{currentView.replace('-', ' ')}</span>
              </div>
              <h2 className="text-2xl font-heading font-extrabold text-white tracking-tight drop-shadow-md">
                CRM Executive<span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-glow to-blue-500"> Desk</span>
              </h2>
            </div>
          </div>

          {/* Center Controls */}
          <div className="flex items-center gap-3 bg-navy-900/80 p-2 rounded-2xl border border-white/5 shadow-[inset_0_2px_15px_rgba(0,0,0,0.4)] backdrop-blur-xl">
            {/* Branch Selector */}
            <div className="relative">
              <button 
                onClick={() => setBranchOpen(!branchOpen)}
                className="flex items-center gap-3 px-5 py-2.5 hover:bg-white/5 rounded-xl text-sm font-semibold transition-all text-gray-200 hover:text-white border border-transparent hover:border-white/5"
              >
                <div className="p-1.5 rounded-lg bg-gradient-to-br from-cyan-glow/20 to-blue-500/20 border border-cyan-glow/30 shadow-[0_0_10px_rgba(0,212,255,0.2)]">
              ...
              {branchOpen && (
                <div className="absolute top-full left-0 mt-4 w-60 glass-panel bg-navy-900/95 backdrop-blur-3xl border border-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.6)] rounded-2xl py-3 z-[100] transform transition-all">
                  <div className="px-5 py-2 border-b border-white/10 mb-2 flex items-center justify-between">
                    <span className="text-[10px] uppercase font-extrabold tracking-widest text-gray-500">Active Regions</span>
                  ...
                  {['All Branches', 'Al Nahda', 'Deira', 'Jumeirah', 'Sharjah', 'Abu Dhabi'].map(branch => (
                    <button 
                      ...
                      className="w-full text-left px-5 py-3 text-sm font-medium text-gray-300 hover:bg-gradient-to-r hover:from-white/10 hover:to-transparent hover:text-white transition-all flex items-center gap-3 group relative overflow-hidden"
                    >
                      <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-0 bg-cyan-glow transition-all group-hover:h-full"></div>
                      <MapPin size={14} className="text-gray-500 group-hover:text-cyan-glow transition-colors" /> {branch}
                    </button>
                  ))}
                </div>
              )}
            ...
            {/* Date Range */}
            <button className="flex items-center gap-3 px-5 py-2.5 hover:bg-white/5 rounded-xl text-sm font-semibold transition-all text-gray-200 hover:text-white border border-transparent hover:border-white/5">
              <div className="p-1.5 rounded-lg bg-gradient-to-br from-purple-500/20 to-pink-500/20 border border-purple-500/30 shadow-[0_0_10px_rgba(192,132,252,0.2)]">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="hidden lg:flex items-center gap-2 bg-white/5 p-1 rounded-xl border border-white/5">
              <button className="p-2.5 hover:bg-white/10 rounded-lg text-gray-400 hover:text-emerald-glow transition-all group" title="Create Lead">
                <Plus size={18} className="group-hover:scale-110 transition-transform" />
              </button>
              <button className="p-2.5 hover:bg-white/10 rounded-lg text-gray-400 hover:text-purple-400 transition-all group" title="Fast Task">
