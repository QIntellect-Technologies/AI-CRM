...
import { Download, FileText, TrendingUp, Users, Calendar, Filter, Search, ChevronDown, PieChart as PieIcon, ActivitySquare, Banknote, ShieldCheck, RefreshCw, Layers, Printer, BarChart3, FileSpreadsheet, ArrowUpRight, CheckCircle2, Clock } from 'lucide-react';
...
export function Reports({ selectedBranch }: { selectedBranch: string }) {
  ...
  if (loading) {
     return (
       <div className="flex flex-col h-[70vh] items-center justify-center gap-6 fade-in">
          <div className="relative">
            <div className="w-20 h-20 border-4 border-cyan-500/20 border-t-cyan-400 rounded-full animate-spin"></div>
            <div className="w-12 h-12 border-4 border-purple-500/20 border-b-purple-400 rounded-full animate-spin-reverse absolute top-4 left-4"></div>
            <BarChart3 className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-cyan-400 animate-pulse" size={24} />
          </div>
          <div className="text-center">
             <h2 className="text-xl font-bold text-white tracking-widest uppercase">Aggregating Data Analytics</h2>
             <p className="text-cyan-400/70 text-sm mt-2 font-mono">Syncing parameters for {selectedBranch} over {dateRange}...</p>
          </div>
       </div>
     );
  }
