...
const marketingData = [
  { name: 'TikTok', leads: 145, ctr: 4.2, cpl: 12 },
  { name: 'Instagram', leads: 95, ctr: 3.8, cpl: 18 },
  { name: 'Snapchat', leads: 65, ctr: 2.5, cpl: 22 },
  { name: 'Google Ads', leads: 40, ctr: 1.8, cpl: 35 },
];
  ...
  return (
    <div className="flex flex-col h-full space-y-6 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        <div className="glass-card p-5 rounded-2xl relative overflow-hidden group border-red-500/30 hover:border-red-500/50 shadow-[0_0_15px_rgba(239,68,68,0.1)]">
          <div className="absolute top-0 right-0 w-full h-full bg-gradient-to-br from-red-500/5 to-transparent pointer-events-none"></div>
          <div className="flex justify-between items-start mb-4">
            <div className="p-2 bg-red-500/10 rounded-lg border border-red-500/20">
              <AlertTriangle size={18} className="text-red-500 drop-shadow-[0_0_5px_rgba(239,68,68,0.8)]" />
            </div>
          </div>
          <p className="text-gray-400 text-sm font-medium mb-1">Inactive Patients</p>
          <div className="flex items-end gap-2">
            <h3 className="text-3xl font-mono font-bold text-red-500 drop-shadow-[0_0_8px_rgba(239,68,68,0.6)]">
              <AnimatedCounter value={42} />
            </h3>
            <span className="text-xs text-red-400 mb-1">Action Required</span>
          </div>
        </div>
      </div>
                ...
                <div key={stage} className="flex-shrink-0 w-64 flex flex-col gap-3">
                  <div className="flex items-center justify-between px-1">
                    ...
                    <span className="text-xs font-mono bg-white/5 px-2 py-0.5 rounded-full text-gray-500">
                      {mockLeads.filter(l => l.status === stage).length}
                    </span>
                  </div>
                  
                  <div className="flex-1 overflow-y-auto hide-scrollbar space-y-3 pr-1">
                    {mockLeads.filter(l => l.status === stage).map(lead => (
                      <div key={lead.id} className="bg-navy-800/80 border border-white/5 rounded-xl p-3 hover:border-cyan-glow/30 transition-colors cursor-pointer group">
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <span className={cn(
                            ...
                            lead.aiScore > 80 ? "bg-red-500/10 text-red-400 border-red-500/20" :
                            lead.aiScore > 50 ? "bg-yellow-500/10 text-yellow-400 border-yellow-500/20" :
                            "bg-blue-500/10 text-blue-400 border-blue-500/20"
                          )}>
                            {lead.aiScore > 80 ? 'HOT' : lead.aiScore > 50 ? 'WARM' : 'COLD'}
                          </span>
                        </div>
