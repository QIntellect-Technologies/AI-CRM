          ...
          {activeTab === 'integrations' && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                {[
                  { name: 'WhatsApp Business API', desc: 'Automated SMS, remiders, and patient re-engagement', icon: Smartphone, status: 'Active', color: 'emerald' },
                  { name: 'TikTok Ads API', desc: 'Sync TikTok lead forms instantly', icon: LinkIcon, status: 'Active', color: 'cyan' },
                  { name: 'Meta (IG/FB) Lead Sync', desc: 'Live syncing of campaign leads into Pipeline', icon: LinkIcon, status: 'Syncing', color: 'blue' },
                  { name: 'Twilio Voice (Call Center)', desc: 'Agent call routing and transcription recordings', icon: Shield, status: 'Active', color: 'emerald' },
                ].map((intg, i) => (
                  <div key={i} className="flex items-center justify-between p-4 bg-navy-900 border border-white/5 rounded-xl">
                    <div className="flex items-center gap-4">
                      <div className={cn("p-2 rounded-lg bg-opacity-20", `bg-${intg.color}-500/20 text-${intg.color}-400`)}>
                        <intg.icon size={24} />
                      </div>
                      <div>
