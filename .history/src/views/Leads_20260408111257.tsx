            ...
            >
              ...
              {sources.map(source => (
                <option key={source} value={source}>{source}</option>
                    ...
                    {viewMode === 'board' && (
        <div className="flex-1 flex gap-6 overflow-x-auto pb-6 pt-4 hide-scrollbar px-2">
          {columns.map((status) => {
            const columnLeads = filteredLeads.filter(l => l.status === status);
            
            // Status-based styling for columns
            let statusColor = "from-gray-500 to-gray-700 text-gray-400";
            let glowBg = "bg-gray-500/5";
            let iconBg = "bg-gray-500/10 text-gray-400 border-gray-500/20";
            
            if (status === 'New Lead') {
              statusColor = "from-blue-400 to-blue-600 text-blue-400";
              glowBg = "bg-blue-500/5";
              iconBg = "bg-blue-500/10 text-blue-400 border-blue-500/20";
            } else if (status === 'Contacted') {
              statusColor = "from-yellow-400 to-orange-500 text-yellow-400";
              glowBg = "bg-yellow-500/5";
              iconBg = "bg-yellow-500/10 text-yellow-400 border-yellow-500/20";
            } else if (status === 'Appointment Set') {
              statusColor = "from-emerald-400 to-emerald-600 text-emerald-400";
              glowBg = "bg-emerald-500/5";
              iconBg = "bg-emerald-500/10 text-emerald-400 border-emerald-500/20";
            } else if (status === 'Converted') {
              statusColor = "from-purple-400 to-purple-600 text-purple-400";
              glowBg = "bg-purple-500/5";
              iconBg = "bg-purple-500/10 text-purple-400 border-purple-500/20";
            } else if (status === 'Inactive') {
              statusColor = "from-red-400 to-red-600 text-red-400";
              glowBg = "bg-red-500/5";
              iconBg = "bg-red-500/10 text-red-400 border-red-500/20";
            }

            return (
              <div key={status} className={cn("flex-1 min-w-[340px] max-w-[360px] flex flex-col rounded-3xl border border-white/5 relative group transition-all", glowBg)}>
                {/* Clean header area */}
                <div className="p-5 flex items-center justify-between bg-navy-900/60 backdrop-blur-xl border-b border-white/5 rounded-t-3xl relative z-10">
                  <div className="flex items-center gap-3">
                    <div className={cn("w-3 h-3 rounded-full shadow-[0_0_10px_rgba(255,255,255,0.5)]", statusColor.split(' ')[0].replace('from-', 'bg-'))}></div>
                    <h3 className={cn("font-heading font-bold text-base tracking-wide text-white")}>
                      {status}
                    </h3>
                  </div>
                  <span className={cn("text-xs font-black px-3 py-1 rounded-xl border shadow-sm", iconBg)}>
                    {columnLeads.length}
                  </span>
                </div>
                
                {/* Column Body */}
                <div className="p-4 flex-1 overflow-y-auto flex flex-col gap-4 hide-scrollbar relative z-10">
                  {columnLeads.map(lead => (
                    <div key={lead.id} className="bg-navy-800/90 backdrop-blur-xl border border-white/10 p-5 rounded-2xl cursor-pointer group/card hover:border-white/20 transition-all hover:shadow-[0_10px_30px_rgba(0,0,0,0.5)] hover:-translate-y-1 relative overflow-hidden" onClick={() => handleLeadClick(lead)}>
                      ...
                      <div className="flex items-start justify-between mb-4 relative z-10">
                        <div className="flex flex-col gap-2">
                          ...
                          <h4 className="font-bold text-white text-lg mt-1 group-hover/card:text-cyan-400 transition-colors drop-shadow-sm">{lead.name}</h4>
                        </div>
                        <button className="text-gray-500 hover:text-white opacity-0 group-hover/card:opacity-100 transition-opacity bg-navy-900/80 p-2 rounded-xl border border-white/10 shadow-sm">
                          <MoreHorizontal size={18} />
                        </button>
                      </div>
                      
                      <div className="flex items-center gap-2.5 mb-5 relative z-10 bg-navy-900/50 p-2.5 rounded-xl border border-white/5">
                        <Phone size={14} className="text-gray-400" />
                        <p className="text-sm text-gray-300 font-medium tracking-wide">{lead.phone}</p>
                      </div>
                      
                      <div className="flex items-center justify-between pt-4 border-t border-white/10 relative z-10">
                        <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-white/5 bg-navy-900/50">
                          <Clock size={12} className="text-gray-400" />
                          <span className="text-xs font-semibold text-gray-400">{lead.last_contact || 'Just now'}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <div className="flex gap-1 opacity-0 group-hover/card:opacity-100 transition-opacity mr-2">
                            <button className="p-2 hover:bg-cyan-500/20 rounded-xl text-cyan-400 transition-all hover:scale-110">
                              <Phone size={14} />
                            </button>
                            <button className="p-2 hover:bg-emerald-500/20 rounded-xl text-emerald-400 transition-all hover:scale-110">
                          ...
                          <div className="flex items-center gap-1.5">
                            <div className={cn(
                              ...
                              lead.ai_score >= 80 ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" :
                            ...
                            <div className={cn(
                              "w-8 h-8 flex items-center justify-center rounded-lg border shadow-sm",
                              priorityColors[lead.priority as keyof typeof priorityColors].replace('text-', 'text-').replace('bg-', 'bg-').split(' ')[0],
                              priorityColors[lead.priority as keyof typeof priorityColors].split(' ')[1].replace('text-', 'border-').replace('400', '500/30'),
                              priorityColors[lead.priority as keyof typeof priorityColors].split(' ')[1]
                            )} title={"Priority: "}>
                              {lead.priority === 'High' ? <AlertTriangle size={14} /> : lead.priority === 'Medium' ? <Clock size={14} /> : <CheckCircle size={14} />}
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  {columnLeads.length === 0 && (
                    <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-white/10 rounded-3xl p-8 bg-navy-800/30 m-2">
                      <div className="w-14 h-14 bg-navy-900/80 rounded-2xl flex items-center justify-center mb-4 shadow-inner border border-white/5">
                        <Users size={24} className="text-gray-500" />
                      </div>
                      <p className="text-sm text-gray-400 text-center font-bold">No leads here</p>
                      <p className="text-xs text-gray-500 text-center mt-1">Drag and drop leads to this stage</p>
                    </div>
                  )}
                </div>
              </div>
