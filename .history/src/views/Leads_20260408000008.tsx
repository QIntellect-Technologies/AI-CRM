    ...
    <div className="space-y-6 max-w-7xl mx-auto h-full flex flex-col pb-10">
      <div className="flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="flex items-center gap-2 px-4 py-2 bg-cyan-glow/10 text-cyan-glow border border-cyan-glow/30 rounded-lg text-sm font-medium hover:bg-cyan-glow/20 transition-all shadow-[0_0_15px_rgba(0,212,255,0.15)] hover:shadow-[0_0_20px_rgba(0,212,255,0.3)]">
            <Plus size={16} />
            Add Lead
          </button>
        </div>
      ...
      {viewMode === 'board' && (
        <div className="flex-1 flex gap-4 overflow-x-auto pb-4 hide-scrollbar">
          {columns.map((status) => {
            const columnLeads = mockLeads.filter(l => l.status === status);
            return (
                    ...
                    <div key={lead.id} className="glass-card p-4 rounded-xl cursor-pointer group hover:border-cyan-glow/30">
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-white/5">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[10px] text-gray-500">{lead.lastContact || 'Just now'}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          <div className={cn(
                            ...
                            lead.aiScore > 80 ? "bg-red-500/10 border-red-500/20 text-red-400" :
                            lead.aiScore > 50 ? "bg-yellow-500/10 border-yellow-500/20 text-yellow-400" :
            ...
            <tbody className="divide-y divide-white/5">
              {mockLeads.map((lead) => (
                <tr key={lead.id} className="glass-table-row group">
                  ...
                  <td className="text-gray-400 text-sm">{lead.assignedTo || '-'}</td>
