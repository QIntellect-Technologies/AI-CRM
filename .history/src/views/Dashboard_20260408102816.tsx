  ...
  return (
    <div className="flex flex-col space-y-8 pb-20 relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
            ...
            {patients.slice(0, 5).map((patient: any, i) => ({
              ...
              <div key={item.id} className="relative flex flex-col md:flex-row items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                <div className="flex items-center justify-center w-10 h-10 rounded-full border border-white/10 bg-navy-800/80 backdrop-blur-sm shadow-[0_0_15px_rgba(0,229,160,0.2)] shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10">
                  <img src={`https://picsum.photos/seed/patient${item.id}/100/100`} alt={item.patient} className="w-8 h-8 rounded-full object-cover border border-white/20" />
                </div>
        ...
        {/* NOTIFICATIONS PANEL */}
        <div className="glass-panel p-8 rounded-3xl relative group overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-bl from-red-500/10 to-transparent blur-3xl opacity-50 pointer-events-none"></div>
          <div className="flex items-center justify-between mb-8 relative z-10 border-b border-white/5 pb-4">
            <h2 className="text-xl font-heading font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400 flex items-center gap-3 drop-shadow-[0_0_10px_rgba(255,255,255,0.1)]">
              <div className="p-2 bg-red-500/20 border border-red-500/30 rounded-xl shadow-[0_0_15px_rgba(239,68,68,0.3)]">
                <Bell size={20} className="text-red-400 drop-shadow-[0_0_8px_rgba(239,68,68,1)]" />
              </div>
              System Alerts
            </h2>
            <button className="text-[10px] font-bold uppercase tracking-widest text-gray-400 hover:text-white transition-colors bg-white/5 hover:bg-white/10 px-3 py-1.5 rounded-lg border border-white/10">Mark all read</button>
          </div>
          <div className="space-y-4 relative z-10">
            {notifications.map((notif: any) => (
              <div key={notif.id} className="flex items-start gap-4 p-4 rounded-2xl bg-navy-800/40 border border-white/5 hover:border-white/20 hover:bg-navy-800/60 transition-all cursor-pointer group hover:-translate-y-1 hover:shadow-lg">
                <div className={cn(
                  "w-2.5 h-2.5 rounded-full mt-1.5 shrink-0",
                  notif.severity === 'red' ? "bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.8)]" :
                  notif.severity === 'yellow' ? "bg-gold-glow shadow-[0_0_10px_rgba(245,166,35,0.8)]" :
                  "bg-emerald-glow shadow-[0_0_10px_rgba(0,229,160,0.8)]"
                )}></div>
                <div className="flex-1">
                  <p className="text-sm text-gray-300 font-medium group-hover:text-white transition-colors leading-relaxed">{notif.message}</p>
                  <span className="text-[10px] text-gray-500 font-mono mt-2 block bg-white/5 w-max px-2 py-0.5 rounded">{notif.time}</span>
                </div>
                <button className="text-gray-500 hover:text-red-400 transition-colors p-1.5 rounded-md hover:bg-red-500/10">
                  <X size={14} />
                </button>
              </div>
            ))}
            {notifications.length === 0 && (
              <div className="flex flex-col items-center justify-center py-10 opacity-50">
                ...
                <p className="text-sm font-mono text-gray-400">All caught up!</p>
              </div>
            )}
          </div>
        </div>
