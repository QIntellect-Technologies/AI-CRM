...
import { ArrowLeft, Download, User, Building, Phone, Mail, Receipt, AlertCircle, FileText, CheckCircle, BarChart3, TrendingUp, ShieldCheck, Printer, Send, MessageSquare, ListPlus, CreditCard, ChevronDown, RefreshCw, X, FilePlus2, Search, Filter, History, HandHeart } from 'lucide-react';
...
export function BillingDetails({ patientId, onBack, selectedBranch }: { patientId: string | number | null, onBack: () => void, selectedBranch: string }) {
  ...
  if (loading) {
    return (
      <div className="flex h-full flex-col items-center justify-center text-cyan-glow gap-4">
        <RefreshCw className="animate-spin text-cyan-glow" size={32} />
        <p className="text-gray-300 font-medium tracking-wider animate-pulse">Synchronizing Deep Financial Ledgers...</p>
      </div>
    );
  }
