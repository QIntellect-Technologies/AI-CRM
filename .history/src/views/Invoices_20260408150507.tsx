...
export function Invoices() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto h-full flex flex-col pb-10">
      ...
      <div className="w-full flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="glass-panel overflow-hidden border border-white/5 rounded-2xl">
            {loading ? (
              <div className="flex-1 flex items-center justify-center p-20 text-cyan-glow gap-3">
                ...
                <table className="w-full text-left border-collapse min-w-[800px]">
                  <thead>
                    <tr className="border-b border-white/10 bg-black/20 text-gray-400 text-xs uppercase tracking-wider">
                      ...
                      <th className="py-4 px-6 font-medium">Patient Attached</th>
                    ...
                    {filteredInvoices.map((invoice, idx) => (
                      <tr 
                        ...
                        onClick={() => setSelectedPatient({
                          ...
                          name: invoice.patient_name || `Patient #${invoice.patient_id}`,
                          phone: invoice.patient_phone || 'Unlinked Protocol',
                          invoices: invoices.filter(inv => inv.patient_id === invoice.patient_id)
                        })}
                      >
                        ...
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                               ...
                               <span className="text-xs text-gray-500 font-mono mt-0.5">{invoice.patient_phone || 'Unlinked Protocol'}</span>
                            </div>
                          </div>
                        </td>
                        <td className="py-4 px-6">
      ...
      {/* Patient Billing History Modal */}
      {selectedPatient && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-navy-950/80 backdrop-blur-md" onClick={() => setSelectedPatient(null)} />
          
          <div className="bg-navy-800 border border-white/10 rounded-2xl w-full max-w-4xl max-h-[85vh] overflow-hidden relative shadow-2xl shadow-cyan-900/20 flex flex-col animate-in fade-in zoom-in-95">
            <div className="flex items-center justify-between p-6 border-b border-white/5 bg-navy-900/50 shrink-0">
              <div>
                 <h2 className="text-xl font-bold text-white flex items-center gap-2">
                   <User className="text-cyan-glow" />
                   {selectedPatient.name} - Billing History
                 </h2>
                 <p className="text-gray-400 text-sm mt-1 flex items-center gap-2">
                   <Clock size={14}/> Complete Financial Ledger
                 </p>
              </div>
              <button onClick={() => setSelectedPatient(null)} className="text-gray-400 hover:text-white transition-colors p-2 rounded-md hover:bg-white/10">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto flex-1">
               <div className="grid grid-cols-3 gap-4 mb-6">
                 <div className="glass-panel p-4 rounded-xl border border-white/5 text-center">
                    ...
                    <p className="text-xl font-bold text-white mt-1">{selectedPatient.invoices.length}</p>
                 </div>
                 <div className="glass-panel p-4 rounded-xl border border-white/5 text-center">
                    <p className="text-gray-400 text-xs">Total Billed</p>
                    <p className="text-xl font-bold text-cyan-400 mt-1">
                      {selectedPatient.invoices.reduce((sum: number, inv: any) => sum + (inv.total_amount || inv.amount || 0), 0).toLocaleString()} <span className="text-xs">AED</span>
                    </p>
                 </div>
                 <div className="glass-panel p-4 rounded-xl border border-white/5 text-center">
                    <p className="text-gray-400 text-xs">Pending Clear</p>
                    <p className="text-xl font-bold text-yellow-400 mt-1">
                      {selectedPatient.invoices.reduce((sum: number, inv: any) => inv.status !== 'Paid' ? sum + (inv.total_amount || inv.amount || 0) : sum, 0).toLocaleString()} <span className="text-xs">AED</span>
                    </p>
                 </div>
               </div>
               ...
               <div className="glass-panel rounded-xl border border-white/5 overflow-hidden">
                 <table className="w-full text-left border-collapse text-sm">
                   ...
                   <tbody className="divide-y divide-white/5">
                     {selectedPatient.invoices.map((inv: any, idx: number) => (
                        <tr key={inv.id || idx} className="hover:bg-white/[0.02] transition-colors">
                           <td className="py-3 px-4 font-mono text-cyan-glow">{inv.invoice_number || `INV-${String(inv.id).padStart(4, '0')}`}</td>
                           <td className="py-3 px-4 text-gray-300">{inv.created_at ? new Date(inv.created_at).toLocaleDateString() : 'N/A'}</td>
                           <td className="py-3 px-4 text-gray-300">{inv.due_date ? new Date(inv.due_date).toLocaleDateString() : 'N/A'}</td>
                           <td className="py-3 px-4 font-bold text-white">{(inv.total_amount || inv.amount || 0).toLocaleString()} <span className="text-xs text-gray-500 font-normal">AED</span></td>
            ...
            <div className="p-6 border-t border-white/5 shrink-0 bg-navy-900 flex justify-between items-center">
               <button className="px-4 py-2 bg-white/5 text-white rounded-lg text-sm hover:bg-white/10 transition flex items-center gap-2">
                 <Download size={14} /> Export Patient Statement
               </button>
               <button onClick={() => setSelectedPatient(null)} className="px-5 py-2 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-lg hover:from-emerald-400 hover:to-teal-400 transition-colors font-medium">
                 Close Ledger
               </button>
            </div>
          </div>
