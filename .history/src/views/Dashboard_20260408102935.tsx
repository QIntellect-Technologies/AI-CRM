        ...
        <div className="glass-panel p-8 rounded-3xl relative group overflow-hidden">
          ...
          <div className="space-y-5 relative z-10">
            {stats.branchStats && stats.branchStats.slice(0, 5).map((branch: any, idx: number) => {
              ...
              return (
                <div key={idx} className="group/branch">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex flex-col">
                      <span className="text-sm font-bold text-gray-200 group-hover/branch:text-white transition-colors">{branch.name}</span>
                      <span className="text-[9px] uppercase tracking-widest text-gray-500">{branch.location}</span>
                    </div>
                    <div className="flex items-center gap-3 text-xs font-mono">
                      <span className="text-cyan-glow bg-cyan-glow/10 px-1.5 py-0.5 rounded border border-cyan-glow/20" title="Leads">{branch.leads_count}L</span>
                      <span className="text-emerald-glow bg-emerald-glow/10 px-1.5 py-0.5 rounded border border-emerald-glow/20" title="Patients">{branch.patients_count}P</span>
                      <span className="text-purple-400 bg-purple-500/10 px-1.5 py-0.5 rounded border border-purple-500/20" title="Appointments">{branch.appointments_count}A</span>
                    </div>
                  </div>
                  <div className="h-1.5 w-full bg-navy-800/50 rounded-full overflow-hidden border border-white/5">
                    <div 
                      className={cn(
