...
export function CallCenter({ onAgentSelect }: { onAgentSelect?: (id: string | number) => void }) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {agents.map(agent => (
              <div 
                ...
                className="cursor-pointer glass-card p-6 rounded-2xl border border-white/5 hover:border-cyan-glow/30 transition-all group relative overflow-hidden"
              >
                <div className="flex justify-between items-start mb-6 relative z-10">
                  <div className="flex gap-4 items-center">
                    <div className="relative">
                      <img src={`https://i.pravatar.cc/150?u=a${agent.id}`} alt={agent.name} className="w-14 h-14 rounded-full border-2 border-white/10 object-cover" />
                  ...
                  <button 
                    ...
                    className={cn("p-2.5 rounded-xl border transition-all shadow-[0_0_10px_rgba(0,0,0,0.2)]", activeWhisper === agent.id ? "bg-cyan-glow/20 text-cyan-glow border-cyan-glow/50" : "bg-white/5 text-gray-400 border-transparent hover:text-cyan-glow hover:bg-cyan-glow/10 hover:border-cyan-glow/30")} 
                    title={activeWhisper === agent.id ? "Stop Listening" : "Listen / Whisper"}
                  >
                    <Headphones size={18}/>
                  </button>
                </div>

                {activeWhisper === agent.id && (
                   <div className="absolute inset-0 bg-navy-950/90 backdrop-blur-sm z-20 flex flex-col items-center justify-center p-6 border border-cyan-glow/30 rounded-2xl animate-in fade-in duration-200">
                     <Headphones className="text-cyan-glow animate-pulse mb-3" size={32}/>
                     <h4 className="text-white font-bold mb-1">Whispering to {agent.name}</h4>
                     <p className="text-xs text-gray-400 text-center mb-4">You are monitoring this call. The agent can hear you, caller cannot.</p>
                     <button onClick={() => setActiveWhisper(null)} className="px-4 py-2 bg-red-500 hover:bg-red-400 text-white rounded-lg font-bold text-sm transition-all shadow-[0_0_15px_rgba(239,68,68,0.4)]">End Session</button>
                   </div>
                )}
                
                {activeBarge === agent.id && (
                   <div className="absolute inset-0 bg-red-950/90 backdrop-blur-sm z-20 flex flex-col items-center justify-center p-6 border border-red-500/50 rounded-2xl animate-in fade-in duration-200">
                     <AlertTriangle className="text-red-500 animate-bounce mb-3" size={32}/>
                     <h4 className="text-white font-bold mb-1">Barged In!</h4>
                     <p className="text-xs text-red-300 text-center mb-4">You have interrupted {agent.name}'s call and are now speaking directly to the caller.</p>
                     <button onClick={() => setActiveBarge(null)} className="px-4 py-2 bg-white/10 hover:bg-white/20 text-white border border-white/20 rounded-lg font-bold text-sm transition-all">Remove Self</button>
                   </div>
                )}
                ...
                <div className="flex items-center justify-between pt-4 border-t border-white/5 relative z-10">
                   ...
                   <div className="flex gap-2">
                     {agent.sentiment === 'Frustrated' && (
                       <button onClick={(e) => { e.stopPropagation(); handleBarge(agent.id); }} className="text-xs font-bold px-3 py-1.5 bg-red-500 text-white rounded-lg transition-all hover:bg-red-400 shadow-[0_0_15px_rgba(239,68,68,0.4)] animate-bounce">
                         Barge In
                       </button>
                     )}
                     <button onClick={(e) => e.stopPropagation()} className="text-xs font-bold p-2 bg-gradient-to-r from-white/5 to-transparent hover:bg-white/10 text-gray-400 border border-white/10 rounded-lg transition-all" title="More Options">
