...
export function CallCenter({ onAgentSelect, selectedBranch }: { onAgentSelect?: (id: string | number) => void, selectedBranch?: string }) {
  ...
  const fetchData = async () => {
    try {
      const [agentsData, recordingsData] = await Promise.all([
        api.getCallAgents().catch(() => []),
        api.getCallRecordings().catch(() => [])
      ]);
