...
export function AgentDetails({ agentId, onBack, selectedBranch }: { agentId: string | number | null, onBack: () => void, selectedBranch?: string }) {
  ...
  if (loading) {
    return <GlobalLoader label="Initiating Agent Core..." subLabel="Loading AI parameters" />;
  }
  ...
  const avgQAScore = totalCalls ? (agentRecordings.reduce((acc, r) => acc + (r.quality_score || 0), 0) / totalCalls * 100) : 0;
  ...
  const chartData = [
    ...
    { name: 'Fri', score: avgQAScore || 90, vol: totalCalls },
  ];
        ...
        <div className="flex flex-col md:flex-row gap-8 relative z-10">
          ...
          <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-navy-900/50 rounded-xl p-4 border border-white/5 flex flex-col justify-center">
              ...
              <div className="text-2xl font-bold text-white">{avgQAScore > 0 ? `${avgQAScore.toFixed(1)}%` : `${agent.performance_score}%`}</div>
            </div>
            <div className="bg-navy-900/50 rounded-xl p-4 border border-white/5 flex flex-col justify-center">
