...
import { Search, Filter, Plus, MoreHorizontal, BrainCircuit, Clock, Phone, Calendar as CalendarIcon, X, Star, AlertTriangle, CheckCircle, Users, TrendingUp, Target, Mail } from 'lucide-react';
...
export function Leads() {
  ...
  useEffect(() => {
    fetchLeads();
    const handleMutation = () => fetchLeads();
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto flex flex-col pb-10">
      ...
      {/* Call Modal */}
      {callingLead && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 shadow-2xl backdrop-blur-md">
          <div className="bg-navy-900 border border-white/10 rounded-2xl w-full max-w-sm shadow-[0_0_50px_rgba(0,0,0,0.7)] flex flex-col items-center py-8 relative scale-100 animate-in fade-in zoom-in duration-300">
             
            <div className="w-24 h-24 rounded-full bg-navy-800 border-2 border-white/5 flex items-center justify-center mb-6 relative">
              {callStatus !== 'Idle' && callStatus !== 'Ended' && (
                <div className="absolute inset-0 rounded-full border border-emerald-500 animate-ping opacity-75"></div>
              )}
              {callStatus === 'Connected' && (
                <div className="absolute inset-[-10px] rounded-full border-2 border-emerald-500/50 animate-pulse"></div>
              )}
              <Phone className={cn("z-10", callStatus === 'Connected' ? 'text-emerald-400' : 'text-gray-400')} size={40} />
            </div>

            <h3 className="text-2xl font-bold tracking-wide text-white mb-2">{callingLead.name}</h3>
            <p className="text-gray-400 text-lg mb-1">{callingLead.phone}</p>
            
            <div className="mt-4 mb-8">
              {callStatus === 'Connecting' && <span className="text-cyan-400 font-medium animate-pulse">Connecting...</span>}
              {callStatus === 'Ringing...' && <span className="text-emerald-400 font-medium animate-pulse">Ringing...</span>}
              {callStatus === 'Connected' && <span className="text-emerald-400 font-bold flex items-center gap-2">Connected <span className="text-xl tabular-nums">{formatDuration(callDuration)}</span></span>}
              {callStatus === 'Ended' && <span className="text-gray-500 font-medium">Call Ended</span>}
            </div>
            ...
            <div className="flex gap-4">
              <button 
                ...
                className="w-16 h-16 rounded-full bg-red-500 hover:bg-red-400 text-white flex items-center justify-center transition-transform hover:scale-110 shadow-[0_0_20px_rgba(239,68,68,0.4)] disabled:opacity-50"
              >
      ...
      {schedulingLead && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-navy-900 border border-white/10 rounded-2xl w-full max-w-lg shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-hidden scale-100 animate-in fade-in zoom-in duration-200">
            <div className="p-5 border-b border-white/5 flex items-center justify-between bg-gradient-to-r from-emerald-500/10 to-cyan-500/10">
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
                <CalendarIcon size={20} className="text-emerald-400" /> Schedule Appointment
              </h2>
              <button onClick={() => setSchedulingLead(null)} className="p-1.5 text-gray-400 hover:text-white rounded-lg hover:bg-white/10 transition-colors">
                <X size={20} />
              </button>
            </div>

            <form onSubmit={handleScheduleSubmit} className="p-6 space-y-5">
              <div className="bg-navy-800/50 p-4 rounded-xl border border-white/5 mb-4">
                <p className="text-sm text-gray-400">Patient</p>
                <p className="text-lg font-bold text-white">{schedulingLead.name}</p>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-sm text-gray-400 font-medium px-1">Date</label>
                  <input
                    type="date"
                    required
                    value={scheduleData.date}
                    onChange={(e) => setScheduleData({ ...scheduleData, date: e.target.value })}
                    className="w-full bg-navy-800/80 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-emerald-400/50 focus:ring-1 focus:ring-emerald-400/50 transition-all [&::-webkit-calendar-picker-indicator]:filter-[invert(1)]"
                  />
                </div>
                <div className="space-y-1.5">
                  <label className="text-sm text-gray-400 font-medium px-1">Time</label>
                  <input
                    type="time"
                    required
              ...
              <div className="space-y-1.5">
                <label className="text-sm text-gray-400 font-medium px-1">Doctor</label>
                <select
                  value={scheduleData.doctor}
                  onChange={(e) => setScheduleData({ ...scheduleData, doctor: e.target.value })}
                  className="w-full bg-navy-800/80 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-cyan-glow/50 focus:ring-1 focus:ring-cyan-glow/50 transition-all appearance-none cursor-pointer"
                >
              ...
              <div className="space-y-1.5">
                <label className="text-sm text-gray-400 font-medium px-1">Appointment Type</label>
                <select
              ...
              <div className="pt-4 flex justify-end gap-3">
                <button type="button" onClick={() => setSchedulingLead(null)} className="px-5 py-2.5 text-gray-400 hover:text-white hover:bg-white/5 rounded-xl font-medium transition-all">Cancel</button>
                <button type="submit" className="px-5 py-2.5 bg-gradient-to-r from-emerald-500 to-emerald-400 hover:from-emerald-400 hover:to-emerald-300 text-white font-bold rounded-xl shadow-[0_0_15px_rgba(16,185,129,0.3)] hover:shadow-[0_0_20px_rgba(16,185,129,0.5)] transition-all hidden sm:block">
                  Confirm Schedule
                </button>
              </div>
            </form>
            ...
            <div className="p-6">
              ...
              <div className="space-y-6">
                ...
                <div className="flex gap-3">
                  <button 
                    onClick={() => { setIsLeadModalOpen(false); startCall(selectedLead); }}
                    className="flex items-center gap-2 px-4 py-2 bg-cyan-glow/10 text-cyan-glow border border-cyan-glow/30 rounded-lg hover:bg-cyan-glow/20 transition-all">
                    <Phone size={16} />
                    Call
                  </button>
                  <button 
                    onClick={() => { setIsLeadModalOpen(false); openScheduleModal(selectedLead); }}
      ...
      {loading ? (
        <div className="flex h-[60vh] items-center justify-center">
            <GlobalLoader label="Fetching Lead Pipeline..." subLabel="Correlating cross-channel interactions" />
        </div>
      ) : (
        <>
                          ...
                          <div className="flex items-center justify-between pt-5 border-t border-white/10 relative z-10">
                            ...
                            <div className="flex items-center gap-2">
                                <button 
                                  ...
                                  className="p-2.5 hover:bg-cyan-500/20 rounded-xl text-cyan-400 bg-navy-900/80 border border-white/10 transition-all hover:scale-110 shadow-[0_0_10px_rgba(0,0,0,0.3)]">
                                  <Phone size={16} />
                                </button>
                                <button 
                                  onClick={(e) => { e.stopPropagation(); openScheduleModal(lead); }}
                                  className="p-2.5 hover:bg-emerald-500/20 rounded-xl text-emerald-400 bg-navy-900/80 border border-white/10 transition-all hover:scale-110 shadow-[0_0_10px_rgba(0,0,0,0.3)]">
                                <CalendarIcon size={16} />
                              </button>
                            </div>
                          </div>
                  ...
                  <tr key={lead.id} className="group/row hover:bg-white/5 transition-colors cursor-pointer" onClick={() => handleLeadClick(lead)}>
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-2 opacity-0 group-hover/row:opacity-100 transition-opacity">
                          <button 
                            onClick={(e) => { e.stopPropagation(); startCall(lead); }}
                            className="p-2 bg-cyan-glow/10 border border-cyan-glow/20 rounded-lg text-cyan-glow hover:bg-cyan-glow/20 transition-colors shadow-sm" title="Call Lead">
                            <Phone size={14} />
                          </button>
                          <button 
                            onClick={(e) => { e.stopPropagation(); openScheduleModal(lead); }}
                            className="p-2 bg-emerald-500/10 border border-emerald-500/20 rounded-lg text-emerald-400 hover:bg-emerald-500/20 transition-colors shadow-sm" title="Schedule Appointment">
